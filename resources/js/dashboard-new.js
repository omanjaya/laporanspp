...
class DashboardApplication {
     ...
     */
    handleWindowResize() {
        // Nothing to do on window resize without charts
    }
    ...
    async refreshAllData() {
        try {
            ...
            await SearchManagerInstance.loadSchools();
...
// Make common functions available globally for template usage
window.showSection = (sectionName) => {
    if (DashboardApp.isInitialized) {
        DashboardApp.getModule('ui').showSection(sectionName);
    } else {
        console.warn('Dashboard not yet initialized, showing section after init...');
        setTimeout(() => window.showSection(sectionName), 100);
    }
};
