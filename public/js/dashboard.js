...
class ErrorHandler {
    ...
    async processQueue() {
        while (this.errorQueue.length > 0 && this.isOnline) {
            ...
            try {
                ...
                console.error('Failed to process queued error:', e);
        ...
        try {
            ...
            if (!response.ok) {
                ...
                if (response.status === 429) {
                    ...
                    throw new Error(`Terlalu banyak permintaan. Silakan coba lagi dalam ${retryAfter} detik.`);
                }
                ...
                if (response.status === 422) {
                    ...
                    const firstError = Object.values(validationErrors)[0]?.[0] || errorData.message;
                    throw new Error(firstError || 'Data yang Anda masukkan tidak valid.');
                }
    ...
    showError(error, context = '') {
        ...
        if (message.includes('fetch')) {
            ...
            message = 'Koneksi internet tidak stabil. Silakan coba lagi.';
        } else if (message.includes('JSON')) {
            message = 'Format data dari server tidak valid. Silakan coba lagi.';
        }

        this.displayError(message, context);
        this.logError('User Error Displayed', { message, context });
    }

    displayError(message, context = '') {
        // Create error notification
        const errorNotification = document.createElement('div');
        errorNotification.className = 'fixed top-4 right-4 bg-red-500 text-white px-6 py-4 rounded-lg shadow-lg z-50 max-w-md transform transition-all duration-300 translate-x-full';
        errorNotification.innerHTML = `
            <div class="flex items-start">
                ...
                <div class="ml-3 flex-1">
                    ...
                    ${context ? `<p class="text-xs mt-1 opacity-90">${context}</p>` : ''}
                </div>
    ...
    showSuccess(message) {
        ...
        successNotification.className = 'fixed top-4 right-4 bg-green-500 text-white px-6 py-4 rounded-lg shadow-lg z-50 max-w-md transform transition-all duration-300 translate-x-full';
...
// Common utility functions (legacy compatibility)
...
function showError(message) {
    errorHandler.showError(new Error(message));
}

function hideError() {
    // Hide legacy error alert if it exists
    const errorAlert = document.getElementById('errorAlert');
    if (errorAlert) errorAlert.classList.add('hidden');
}

// Dashboard Analytics functionality with enhanced error handling
async function loadDashboardAnalytics() {
    try {
        const result = await errorHandler.makeRequest('/api/dashboard/analytics');
        ...
        errorHandler.showError(error, 'Gagal memuat data dashboard');
    }
}
...
async function uploadFile() {
    if (!selectedFile) {
        errorHandler.showError(new Error('Pilih file terlebih dahulu'), 'Import Data');
        return;
    }
    ...
    try {
        const result = await errorHandler.makeRequest('/api/rekon/import', {
            method: 'POST',
            body: formData,
            headers: {
        ...
        if (result.success) {
            ...
            errorHandler.showSuccess(`Berhasil mengimport ${result.imported} data`);
        } else {
            errorHandler.showError(new Error(result.message), 'Import Data');
        }
        ...
        errorHandler.showError(error, 'Import Data Gagal');
    }
}
...
async function uploadBankFile() {
    if (!selectedBankFile) {
        errorHandler.showError(new Error('Pilih file CSV Bank terlebih dahulu'), 'Import CSV Bank');
        return;
    }
    ...
    try {
        const result = await errorHandler.makeRequest('/api/rekon/import-bank', {
            method: 'POST',
            body: formData,
            headers: {
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]')?.getAttribute('content') || ''
        ...
        if (result.success) {
            ...
            // Refresh dashboard analytics
            loadDashboardAnalytics();
            errorHandler.showSuccess(`Berhasil mengimport ${result.imported} data dari CSV Bank`);
        } else {
            errorHandler.showError(new Error(result.message), 'Import CSV Bank');
        }
        ...
        errorHandler.showError(error, 'Import CSV Bank Gagal');
    }
}
